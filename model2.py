...
class SelfAttentionNetwork(Module):
    def __init__(self, opt, n_node):
        ...
        self.transformerEncoder = Mega(dim=self.hidden_size, ema_heads=opt.nhead, attn_dim_qk=64, num_tokens = 8,  depth = 6)
        #self.transformerEncoder = Mega(dim=128, ema_heads=16, attn_dim_qk=64, num_tokens = 256,  depth = 6)
        # print(self.n_node)
    ...
    def forward(self, i):
        # print(inputs.size())
        hidden = self.transformerEncoder(i)
        # hidden = hidden.transpose(0,1).contiguous()
        # print(hidden.size())
